(function () {
  ...
  function formatPhone(n) {
    return n.replace(/(\d{3})(\d{2})(\d{3})(\d{2})(\d{2})/, '$1 $2 $3 $2 $3');
  }
  ...
  function renderContacts() {
      ...
      .map(
        n => `
          <a
            ...
            href="${waLink(
              n,
              'Bonjour Leader Automobile, j’ai une question.'
            )}"
          >
            Discuter sur WhatsApp · +${formatPhone(n)}
          </a>
        `
      )
      .join('');
    ...
    const link = waLink(
      primary,
      'Bonjour Leader Automobile, je souhaite avoir plus d’informations.'
    );
  ...
  function showDbError() {
    ...
    if (empty) {
      ...
      empty.querySelector('p').textContent =
        'Vérifie la configuration Supabase ou réessaie dans quelques instants.';
    }
  }
...
`;
      ...
      .map(
        n => `
          <a
            ...
            href="${waLink(n, waMsg)}"
          >
            WhatsApp +${formatPhone(n).split(' ').slice(-2).join(' ')}
          </a>
        `
      )
      .join('');
    ...
    return `
      ...
      >
        ...
        >
          ...
          <span
            class="badge badge-${escapeHTML(v.category)}"
          >
              ...
              v.mileage !== undefined
                ? `
                  <span class="spec-pill">
                    ${Number(v.mileage).toLocaleString('fr-FR')} km
                  </span>
                `
                : ''
            }
   ...
   * =========================================================
   *
   * Les 3 voitures utilisées en arrière-plan :
   *
   * 1. Ford Fusion
   * 2. Hyundai Sonata
   * 3. Kia K7
   *
   * Elles sont indépendantes du catalogue Supabase.
   */
  ...
  function renderHeroVehicles() {
    ...
    const heroCars = [
      {
        name: 'Ford Fusion',
        src: 'https://upload.wikimedia.org/wikipedia/commons/7/72/2017_Ford_Fusion_in_Ingot_Silver%2C_front_right%2C_02-14-2026.jpg'
      },

      {
        name: 'Hyundai Sonata',
        src: 'https://upload.wikimedia.org/wikipedia/commons/8/8d/2018_Hyundai_Sonata_SE%2C_Front_Left%2C_06-07-2020.jpg'
      },

      {
        name: 'Kia K7',
        src: 'https://upload.wikimedia.org/wikipedia/commons/4/46/00_KIA_K7_1.jpg'
      }
    ];

    hero.innerHTML = heroCars
      .map(
        (car, i) => `
          <img
            class="hero-car hero-car-${i + 1}"
            src="${escapeHTML(car.src)}"
  ...
  function renderArrivals() {
    ...
    const list = state.vehicles.filter(
      v => v.category === 'arrivage'
    );
